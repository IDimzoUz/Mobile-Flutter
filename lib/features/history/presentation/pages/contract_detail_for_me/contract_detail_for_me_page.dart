...
class ContractDetailForMePage extends StatefulWidget {
  const ContractDetailForMePage({super.key, this.dataResponse});
  final ForMeHistoryResponse? dataResponse;
  @override
  State<ContractDetailForMePage> createState() => _PageState();
}

class _PageState extends State<ContractDetailForMePage> {
    ...
    child: LifecycleAware(
      ...
      builder: (BuildContext context, Lifecycle lifecycle) => BlocBuilder<ContractDetailBloc, ContractDetailState>(
        ...
        builder: (context, state) => Scaffold(
          ...
          bottomNavigationBar: state.status == ApiStatus.loading || state.response == null
              ? const SizedBox()
              : Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Visibility(
                      visible: !(state.response?.recipientApproved ?? false),
                      child: Padding(
