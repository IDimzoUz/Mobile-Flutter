...
class CreateFormalizationDetailPage extends StatefulWidget {
  CreateFormalizationDetailPage({super.key, this.response});
  late CreateContractsResponse? response;
  @override
...
class _PageState extends State<CreateFormalizationDetailPage> {
  ...
  Future<void> alertVerify(String status) async {
    switch (status) {
      case "PENDING":
        context.pushNamed(Routes.selectPaymentVerifyPage, extra: widget.response);
        break;
  ...
  Widget build(BuildContext context) => BlocListener<CreateFormalizationDetailBloc, CreateFormalizationDetailState>(
    listener: (BuildContext context, CreateFormalizationDetailState state) async {
      if (state.createContractsResponse != null) {
        widget.response = state.createContractsResponse;
        if (widget.response?.status != "PENDING")  alertVerify(state.createContractsResponse?.status ?? "");
      }
    },
    listenWhen: (CreateFormalizationDetailState p, CreateFormalizationDetailState c) => p.createContractsResponse != c.createContractsResponse,
    child: LifecycleAware(
      observer: LifecycleObserver(
          onVisible: (Lifecycle l) {
             context.read<CreateFormalizationDetailBloc>().add(GetContractDetailIdEvent(id: widget.response?.contractId ?? 0));
          }
      ),
      builder: (BuildContext context, Lifecycle lifecycle) => BlocBuilder<CreateFormalizationDetailBloc, CreateFormalizationDetailState>(
        ...
        builder: (context, state) => Scaffold(
          ...
          child: CustomButton(
            onPressed: () => alertVerify(widget.response?.status ?? ""),
            width: double.infinity,
