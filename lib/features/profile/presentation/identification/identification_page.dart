...
// https://idimzo.uz/success?code=eacfd1ea-6a8b-40f7-9921-7227ff79f173&state=new
...
class _PageState extends State<IdentificationPage> {
  ...
  String? _error;
  MyIdResult? _result;
  late StreamSubscription<dynamic>? _appStreamLinks;
  ...
  void _initUniLinks() {
    _appStreamLinks = appLinks.uriLinkStream.listen((link) {
        debugPrint('link: $link');
        final url = link.toString();
        final code = link.queryParameters['code'];
        if (url.contains('success') && code != null) {
          // context.read<LoginBloc>().add();
          debugPrint('link code: $code');
        }
      },
    );
  }
  ...
  Widget build(BuildContext context) => BlocListener<IdentificationBloc, IdentificationState>(
    listener: (BuildContext context, IdentificationState state) async {
      ...
      if (state.oneIDUrl != null) {
        final url = Uri.parse(state.oneIDUrl ?? "");
        if (await canLaunchUrl(url)) {
          ...
          debugPrint(
              'Error occurred while opening url: $url');
        }
      }
    ...
    child: LifecycleAware(
      ...
      builder: (BuildContext context, Lifecycle lifecycle) => BlocBuilder<IdentificationBloc, IdentificationState>(
        buildWhen: (p, n) => p.oneIDUrl != n.oneIDUrl,
                      ...
                      GestureDetector(
                        onTap: () {
                          setState(() {
                            ...
                            state.oneIDUrl = null;
                            context.read<IdentificationBloc>().add(const GetOneIdUrlEvent());
                          });
                        },
